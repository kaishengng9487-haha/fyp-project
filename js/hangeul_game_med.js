...
const hangeul_LEVELS = [
  ...
  {name: "ㅈ", radius: JAMO_RADIUS, color: "#b492cc", mass: 2},
  {name: "ㅁ", radius: JAMO_RADIUS, color: "#8ecae6", mass: 2},
  {name: "ㅅ", radius: JAMO_RADIUS, color: "#f87171", mass: 2},
  {name: "ㅊ", radius: JAMO_RADIUS, color: "#71f8b9", mass: 2},
  ...
  {name: "사", radius: SYL_RADIUS, color: "#5a0fd3", mass: 2},
  {name: "자", radius: SYL_RADIUS, color: "#22c55e", mass: 2},
  {name: "소", radius: SYL_RADIUS, color: "#f3be2b", mass: 2},
  {name: "무", radius: SYL_RADIUS, color: "#02804f", mass: 2},
  {name: "차", radius: SYL_RADIUS, color: "#ab47bc", mass: 2},
  {name: "손", radius: SYL_RADIUS, color: "#ff9800", mass: 2},
  {name: "물", radius: SYL_RADIUS, color: "#1976d2", mass: 2},
  // Words
  {name: "나무", radius: WORD_RADIUS, color: "#b4e197", mass: 2},
  {name: "사자", radius: WORD_RADIUS, color: "#eebf33", mass: 2}
];

// Merge rules for medium
const MERGE_RULES = [
  {from: ["ㄴ","ㅏ"], to: "나"},
  {from: ["ㅅ","ㅏ"], to: "사"},
  {from: ["ㅈ","ㅏ"], to: "자"},
...
// Target words for medium
const TARGET_WORDS = [
  {name: "나무", display: "나무 namu (Tree)", id: "word1", syllables: ["나","무"], jamos: ["ㄴ","ㅏ","ㅁ","ㅜ"]},
  {name: "물", display: "물 mul (Water)", id: "word2", syllables: ["무","ㄹ"], jamos: ["ㅁ","ㅜ","ㄹ"]},
  {name: "사자", display: "사자 saja (Lion)", id: "word3", syllables: ["사","자"], jamos: ["ㅅ","ㅏ","ㅈ"]},
  {name: "차", display: "차 cha (car)", id: "word4", syllables: ["ㅊ","ㅏ"], jamos: ["ㅊ","ㅏ"]},
  {name: "손", display: "손 son (Hand)", id: "word5", syllables: ["소","ㄴ"], jamos: ["ㅅ","ㅗ","ㄴ"]}
];
...
// Always keep 'ㅏ', 'ㅅ', 'ㄴ', 'ㅁ', 'ㅜ' in dropping list, never exclude
    ...
    if (completedTargets[target.name]) {
      if (Array.isArray(target.jamos)) {
        for (const jamo of target.jamos) {
          // Do NOT exclude 'ㅏ', 'ㅅ', 'ㄴ', 'ㅁ', 'ㅜ'
          if (jamo !== "ㅏ" && jamo !== "ㅅ" && jamo !== "ㄴ" && jamo !== "ㅁ" && jamo !== "ㅜ") {
            excluded.add(jamo);
          }
        }
      }
...
// Fix: Also update display for word5 ("손") status
...
// Always keep "무" from being destroyed from syllables, and "ㅏ", "ㅅ", "ㄴ", "ㅁ", "ㅜ" from jamos
  ...
  for (const target of TARGET_WORDS) {
    if (completedTargets[target.name]) {
      ...
      if (Array.isArray(target.jamos)) {
        for (const jamo of target.jamos) {
          // Exclude "ㅏ", "ㅅ", "ㄴ", "ㅁ", "ㅜ"
          if (jamo === "ㅏ" || jamo === "ㅅ" || jamo === "ㄴ" || jamo === "ㅁ" || jamo === "ㅜ") continue;
          let jidx;
          do {

...
// Most commonly used JAMOS for hard mode - always in drop list except when their relevant word is completed!
const COMMON_JAMOS = ["ㄴ", "ㄹ", "ㅁ", "ㅅ", "ㅏ", "ㅓ", "ㅗ", "ㅜ", "ㅣ", "ㅎ"];
const JAMOS = COMMON_JAMOS;

const COMMON_SYLLABLES = ["나", "라", "리", "머", "하", "마", "소", "수", "무"];

const hangeul_LEVELS = [
  // JAMOS
  {name: "ㄴ", radius: JAMO_RADIUS, color: "#f3969a", mass: 2},
  {name: "ㄹ", radius: JAMO_RADIUS, color: "#78c2ad", mass: 2},
  {name: "ㅁ", radius: JAMO_RADIUS, color: "#8ecae6", mass: 2},
  {name: "ㅅ", radius: JAMO_RADIUS, color: "#f87171", mass: 2},
  {name: "ㅏ", radius: JAMO_RADIUS, color: "#ffb980", mass: 2},
  {name: "ㅓ", radius: JAMO_RADIUS, color: "#f3be2b", mass: 2},
  {name: "ㅗ", radius: JAMO_RADIUS, color: "#90be6d", mass: 2},
  {name: "ㅜ", radius: JAMO_RADIUS, color: "#f9c74f", mass: 2},
  {name: "ㅣ", radius: JAMO_RADIUS, color: "#b492cc", mass: 2},
  {name: "ㅎ", radius: JAMO_RADIUS, color: "#d1d5fa", mass: 2},
  // Syllables
  {name: "나", radius: SYL_RADIUS, color: "#3b82f6", mass: 2},
  {name: "라", radius: SYL_RADIUS, color: "#eebf33", mass: 2},
  {name: "리", radius: SYL_RADIUS, color: "#b4e197", mass: 2},
  {name: "머", radius: SYL_RADIUS, color: "#1976d2", mass: 2},
  {name: "하", radius: SYL_RADIUS, color: "#ab47bc", mass: 2},
  {name: "마", radius: SYL_RADIUS, color: "#22c55e", mass: 2},
  {name: "소", radius: SYL_RADIUS, color: "#ff9800", mass: 2},
  {name: "수", radius: SYL_RADIUS, color: "#5a0fd3", mass: 2},
  {name: "무", radius: SYL_RADIUS, color: "#02804f", mass: 2},
  // Words
  {name: "나라", radius: WORD_RADIUS, color: "#5a0fd3", mass: 2},
  {name: "머리", radius: WORD_RADIUS, color: "#eebf33", mass: 2},
  {name: "하마", radius: WORD_RADIUS, color: "#b4e197", mass: 2},
  {name: "소나무", radius: WORD_RADIUS, color: "#f3be2b", mass: 2},
  {name: "수리", radius: WORD_RADIUS, color: "#22c55e", mass: 2},
  {name: "소리", radius: WORD_RADIUS, color: "#ff9800", mass: 2},
];
...
const TARGET_WORDS = [
  {name: "나라", display: "나라 nara (Nation)", id: "word1", syllables: ["나", "라"], jamos: ["ㄴ", "ㄹ", "ㅏ"]},
  {name: "머리", display: "머리 meori (Head)", id: "word2", syllables: ["머", "리"], jamos: ["ㅁ", "ㄹ", "ㅓ", "ㅣ"]},
  {name: "하마", display: "하마 hama (Hippo)", id: "word3", syllables: ["하", "마"], jamos: ["ㅎ", "ㅁ", "ㅏ"]},
  {name: "소나무", display: "소나무 sonamu (Pine tree)", id: "word4", syllables: ["소", "나", "무"], jamos: ["ㅅ", "ㄴ", "ㅏ", "ㅁ", "ㅗ", "ㅜ"]},
  {name: "수리", display: "수리 suri (Repair)", id: "word5", syllables: ["수", "리"], jamos: ["ㅅ", "ㄹ", "ㅜ", "ㅣ"]},
  {name: "소리", display: "소리 sori (Sound)", id: "word6", syllables: ["소", "리"], jamos: ["ㅅ", "ㄹ", "ㅗ", "ㅣ"]},
];
...
// Only keep COMMON_JAMOS in dropping list, except "ㅎ" if its word "하마" is completed
    ...
    if (completedTargets[target.name]) {
      if (Array.isArray(target.jamos)) {
        for (const jamo of target.jamos) {
          // For "ㅎ", only exclude after "하마" is completed
          if (jamo === "ㅎ" && target.name === "하마") {
            excluded.add(jamo);
          } else if (!COMMON_JAMOS.includes(jamo)) {
            excluded.add(jamo);
          }
        }
      }
...
// Destroy all relevant syllables and jamos for the completed target
    ...
    if (completedTargets[target.name]) {
      ...
      if (Array.isArray(target.syllables)) {
        for (const syll of target.syllables) {
          // Exclude COMMON_SYLLABLES
          if (COMMON_SYLLABLES.includes(syll)) continue;
          let sidx;
          do {
            sidx = hangeulObjects.findIndex(obj => hangeul_LEVELS[obj.type].name === syll);
            if (sidx !== -1) hangeulObjects.splice(sidx, 1);
          } while (sidx !== -1);
      ...
      if (Array.isArray(target.jamos)) {
        for (const jamo of target.jamos) {
          // Exclude COMMON_JAMOS, but DO NOT exclude "ㅎ" if target.name is "하마"
          if (jamo === "ㅎ" && target.name === "하마") {
            // Remove "ㅎ" from board and drop list after "하마" is done
          } else if (COMMON_JAMOS.includes(jamo)) continue;
          let jidx;
          do {
...
function inithangeulGame() {
  ...
  // Dynamically set target word labels and status
  for(let i=0;i<TARGET_WORDS.length;++i){
    let el = document.getElementById('hangeul-target-word'+(i+1));
    if(el) el.textContent = TARGET_WORDS[i].display;
    let statusEl = document.getElementById('hangeul-status-'+TARGET_WORDS[i].id);
    if(statusEl){
      statusEl.textContent = completedTargets[TARGET_WORDS[i].name] ? "Completed!" : "Incomplete";
      statusEl.className = "hangeul-target-status " + (completedTargets[TARGET_WORDS[i].name] ? "hangeul-achieved" : "hangeul-not-achieved");
    }
  }
